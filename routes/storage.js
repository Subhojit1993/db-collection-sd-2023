...
storageRoutes.patch(`${storageUrl}:id`, (req, res) => {
    ...
    fs.readFile(storagePath, 'utf-8', (_error, _data) => {
        ...
        let existingCollectionTypeData = [...existingDataCollection[type]];
        // get the data-object related to the specified type
        let existingData = existingCollectionTypeData.find(element => element._id === parseInt(id));
        // if no data matching the product then send failed response status with the message
        if (!existingData) return res.status(400).send({ error: true, msg: `Unable to update the ${type}` })
        ...
        existingCollectionTypeData = existingCollectionTypeData.map(data => data._id === existingData._id ? modifiedExistingData : data);
...
storageRoutes.delete(`${storageUrl}:id`, (req, res) => {
    ...
    fs.readFile(storagePath, 'utf-8', (_error, _data) => {
        ...
        let existingCollectionTypeData = [...existingDataCollection[type]];
        // get the data-object related to the specified type
        let deletedData = existingCollectionTypeData.find(element => element._id === parseInt(id));

        // if no data matching the product then send failed response status with the message
        if (!deletedData) return res.status(400).send({ error: true, msg: `Unable to delete the ${type}` })
        // filter collection data
        existingCollectionTypeData = existingCollectionTypeData.filter(element => element._id !== deletedData._id);
